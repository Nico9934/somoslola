/**
 * CARDS - Patrones de tarjetas y contenedores
 */

export const cards = {
    // Card básica con borde
    bordered: "border border-gray-300 p-6",
    borderedLg: "border border-gray-300 p-8",

    // Card con fondo gris
    background: "bg-gray-50 border border-gray-300 p-6",

    // Card con borde punteado (dropzone)
    dashed: "border-2 border-dashed border-gray-300 rounded-lg p-6",

    // Card con sombra
    shadow: "border border-gray-200 p-6 shadow-sm hover:shadow-md transition-shadow",

    // Card clickeable
    interactive: "border border-gray-300 p-6 cursor-pointer hover:border-black transition-colors",
};
